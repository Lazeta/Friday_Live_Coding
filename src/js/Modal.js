export class Modal {
    ...
    buildModal(content) {
        // Overlay
        // this.overlay = document.createElement('div'); //сократили код нижней строчкой
        // this.overlay.classList.add('.overlay'); //сократили код нижней строчкой
        this.overlay = this.createDomNode(this.overlay, 'div', 'modal', 'overlay', 'overlay_modal');
    ...
    createDomNote (node, element, ...classes) {
        noda = document.createElement(element);
        noda.classList.add(...classes);
        return node
    };
    ...
    closeModal(e) {
        ...
        if(classes.conains('overlay') || classes.contains('modal__close-icon')) {
